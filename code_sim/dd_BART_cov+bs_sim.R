...
bart_cov_bs_gform_func <- function(m){
  ...
  X_covmodel1 <- dffull[!is.na(dffull$lag_logE) & !is.na(dffull$lag_L1),] %>%
    mutate(t0 = as.factor(t0)) %>%
    select(lag_A, lag_logE, lag_L1, logE, t0) %>%
    as.data.frame() %>%
    model.matrix( ~ .-1, data = .)
  
  ## this is to find the Y's for the generative model of L1
  ### it requires L1 to be non-NA b/c L1 depends on L1
  Y_covmodel1 <- dffull[!is.na(dffull$lag_logE) & !is.na(dffull$lag_L1),]$L1
  
  ## this is to fit the generative model of L1
  invisible(capture.output(bart_covmodel1 <- 
    pbart(X_covmodel1, Y_covmodel1, nskip = 1000, ndpost = 10000, ntree = nt,
          nkeeptest = 1, nkeeptrain = 1, nkeeptreedraws = 1)))
  ...
  X_covmodel2 <- dffull[!is.na(dffull$lag_logE) & !is.na(dffull$lag_L1) & 
                          !is.na(dffull$lag_L2),] %>%
    mutate(t0 = as.factor(t0)) %>%
    select(lag_A, lag_logE, lag_L1, lag_L2, logE, L1, t0) %>%
    as.data.frame() %>%
    model.matrix( ~ .-1, data = .)
  
  ## this is to find the Y's for the generative model of L2
  Y_covmodel2 <- dffull[!is.na(dffull$lag_logE) & !is.na(dffull$lag_L1) & 
                          !is.na(dffull$lag_L2),]$L2
  ...
  X_covmodel3 <- dffull[!is.na(dffull$lag_logE) & !is.na(dffull$lag_L1) & 
                          !is.na(dffull$lag_L2) & !is.na(dffull$lag_L3),] %>%
    mutate(t0 = as.factor(t0)) %>%
    select(lag_A, lag_logE, lag_L1, lag_L2, lag_L3, logE, L1, L2, t0) %>%
    as.data.frame() %>%
    model.matrix( ~ .-1, data = .)
  
  Y_covmodel3 <- dffull[!is.na(dffull$lag_logE) & !is.na(dffull$lag_L1) & 
                          !is.na(dffull$lag_L2) & !is.na(dffull$lag_L3),]$L3
  ...
  X_covmodel4 <- dffull[!is.na(dffull$lag_logE) & !is.na(dffull$lag_L1) & 
                          !is.na(dffull$lag_L2) & !is.na(dffull$lag_L3) & 
                          !is.na(dffull$lag_L4),] %>%
    mutate(t0 = as.factor(t0)) %>%
    select(lag_A, lag_logE, lag_L1, lag_L2, lag_L3, lag_L4, logE, L1, L2, L3, t0) %>%
    as.data.frame() %>%
    model.matrix( ~ .-1, data = .)
  
  ## this is to find the Y's for the generative model of L4
  Y_covmodel4 <- dffull[!is.na(dffull$lag_logE) & !is.na(dffull$lag_L1) & 
                          !is.na(dffull$lag_L2) & !is.na(dffull$lag_L3) & 
                          !is.na(dffull$lag_L4),]$L4
  ...
  for (i in 1:N){
    L10 <- L20 <- L30 <- L40 <- logE0 <- A0 <- Y1 <-
      L11 <- L21 <- L31 <- L41 <- logE1 <- A1 <- Y2 <- 
      L12 <- L22 <- L32 <- L42 <- logE2 <- A2 <- Y3 <- as.numeric(rep(NA, N))
    ...
    invisible(capture.output(bart_predict_Y1_prob <-
      predict(bart_ymodel, newdata =
        data.frame(A = A0, L1 = L10, L2 = L20, L3 = L30, L4 = L40, logE = logE0, 
    ...
    invisible(capture.output(bart_predict_logE1_mean <-
      predict(bart_emodel, newdata = data.frame(lag_A = A0, lag_logE = logE0, 
                                                t00 = 1, t01 = 0))[1,]))
    ...
    invisible(capture.output(bart_predict_t1_L1_prob <-
      predict(bart_covmodel1, newdata = 
                data.frame(lag_A = A0, lag_logE = logE0, lag_L1 = L10, 
                           logE = logE1, t00 = 1, t01 = 0))$prob.test.mean))
    ...
    invisible(capture.output(bart_covmodel2_predict_t1_L2_mean <-
      predict(bart_covmodel2, newdata =
        data.frame(lag_A = A0, lag_logE = logE0, lag_L1 = L10, lag_L2 = L20, 
                   logE = logE1, L1 = L11, t00 = 1, t01 = 0))[1,]))
    ...
    invisible(capture.output(bart_covmodel3_predict_t1_L3_mean <-
      predict(bart_covmodel3, newdata =
        data.frame(lag_A = A0, lag_logE = logE0, lag_L1 = L10, lag_L2 = L20, lag_L3 = L30, 
                   logE = logE1, L1 = L11, L2 = L21, t00 = 1, t01 = 0))[1,]))
    ...
    invisible(capture.output(bart_covmodel4_predict_t1_L4_mean <-
      predict(bart_covmodel4, newdata =
        data.frame(lag_A = A0, lag_logE = logE0, lag_L1 = L10, lag_L2 = L20, lag_L3 = L30, 
                   lag_L4 = L40, logE = logE1, L1 = L11, L2 = L21, L3 = L31, 
                   t00 = 1, t01 = 0))[1,]))
    ...
    invisible(capture.output(bart_predict_Y2_prob <-
      predict(bart_ymodel, newdata =
        data.frame(A = A1, L1 = L11, L2 = L21, L3 = L31, L4 = L41, logE = logE1, 
    ...
    invisible(capture.output(bart_predict_t2_L1_prob <-
      predict(bart_covmodel1, newdata = 
                data.frame(lag_A = A1, lag_logE = logE1, lag_L1 = L11, 
                           logE = logE2, t00 = 0, t01 = 1))$prob.test.mean))
    L12[uY] <- rbinom(sum(uY), 1, bart_predict_t2_L1_prob[uY])
    
    ## generate L2
    invisible(capture.output(bart_covmodel2_predict_t2_L2_mean <-
      predict(bart_covmodel2, newdata =
        data.frame(lag_A = A1, lag_logE = logE1, lag_L1 = L11, lag_L2 = L21, 
                   logE = logE2, L1 = L12, t00 = 0, t01 = 1))[1,]))
    ...
    invisible(capture.output(bart_covmodel3_predict_t2_L3_mean <-
      predict(bart_covmodel3, newdata =
        data.frame(lag_A = A1, lag_logE = logE1, lag_L1 = L11, lag_L2 = L21, lag_L3 = L31, 
                   logE = logE2, L1 = L12, L2 = L22, t00 = 0, t01 = 1))[1,]))
    ...
    invisible(capture.output(bart_covmodel4_predict_t2_L4_mean <-
      predict(bart_covmodel4, newdata =
        data.frame(lag_A = A1, lag_logE = logE1, lag_L1 = L11, lag_L2 = L21, lag_L3 = L31, 
                   lag_L4 = L41, logE = logE2, L1 = L12, L2 = L22, L3 = L32, 
                   t00 = 0, t01 = 1)[1,])))
    ...
    invisible(capture.output(bart_predict_Y3_prob <-
      predict(bart_ymodel, newdata =
        data.frame(A = A2, L1 = L12, L2 = L22, L3 = L32, L4 = L42, logE = logE2, 
