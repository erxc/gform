...
datagen_dd_nonlinear <- function(i, K, beta0, beta1, beta2, beta3,
  ...
  A <- rbinom(1, 1, plogis(xi0 + xi1*L1*cos(xi2*L2) + xi3*L2*L3))
  ...
  # Y <- rbinom(1, 1, plogis(theta0 + theta1*A + log(abs(theta2*L1 + theta3*cos(L2*L3))) - theta3*(L1*L2^2)))
  ...
  if (temp$C == 1) {
    ...
    for (j in 2:K) {
      ...
      Astar <- rbinom(1, 1, plogis(xi0 + xi1*L1star*cos(xi2*L2star) + xi3*L2star*L3star))
      ...
      if (Cstar == 1) {
        ...
        Ystar <- rbinom(1, 1, plogis(theta0 + theta1*Astar + theta2*L1star + theta3*L2star*L3star -
                                       theta3*(L1star*L2star^2) + theta4*t0))
      }
