...
bart_bs_gform_func <- function(m){
  ...
  # treatment model
  xi0 <- -0.5; xi1 <- -1; xi2 <- 0.75; xi3 <- -0.5
  
  # exposure model
  eta0 <- -3; eta1 <- -1; eta2 <- 0.75; eta3 <- -0.5; eta4 <- 0 # 20% censoring
  # eta0 <- -1.5; eta1 <- -1; eta2 <- 0.75; eta3 <- -0.5; eta4 <- 0 # 50% censoring
  ...
  invisible(capture.output(bart_amodel <-
                             lbart(X_amodel, Y_amodel, nskip = 10000, ndpost = 15000, ntree = nt,
  ...
  invisible(capture.output(bart_cmodel <-
                             lbart(X_cmodel, Y_cmodel, nskip = 10000, ndpost = 15000, ntree = nt, 
                                   nkeeptest = 1, nkeeptrain = 1, nkeeptreedraws = 1)))
  ...
  X_emodel <- dffull[!is.na(dffull$lag_logE) & !is.na(dffull$lag_L2),] %>%
    mutate(t0 = as.factor(t0)) %>%
    select(lag_A, lag_logE, lag_L2, t0) %>%
    as.data.frame() %>%
    model.matrix( ~ .-1, data = .)
  
  Y_emodel <- dffull[!is.na(dffull$lag_logE) & !is.na(dffull$lag_L2),]$logE
  
  invisible(capture.output(bart_emodel <- 
                             wbart(X_emodel, Y_emodel, nskip = 10000, ndpost = 15000, ntree = nt,
  ...
  X_covmodel2 <- dffull[!is.na(dffull$lag_logE) & !is.na(dffull$lag_L2),] %>%
    mutate(t0 = as.factor(t0)) %>%
    select(lag_A, lag_logE, logE, lag_L2, t0) %>%
    as.data.frame() %>%
    model.matrix( ~ .-1, data = .)
  ...
  invisible(capture.output(bart_covmodel2 <- 
                             wbart(X_covmodel2, Y_covmodel2, nskip = 10000, ndpost = 15000, ntree = nt,
                                   nkeeptest = 1, nkeeptestmean = 1, nkeeptrain = 1, nkeeptreedraws = 1)))
  ...
  invisible(capture.output(bart_ymodel <-
                             lbart(X_ymodel, Y_ymodel, nskip = 10000, ndpost = 15000, ntree = nt,
                                   nkeeptest = 1, nkeeptrain = 1, nkeeptreedraws = 1)))
  ...
  for (i in 1:N){
    logE0 <- L20 <- A0 <- Y1 <-
      ...
      logE2 <- L22 <- A2 <- Y3 <-
      logE3 <- L23 <- A3 <- Y4 <-
      logE4 <- L24 <- A4 <- Y5 <- as.numeric(rep(NA, N))
    ...
    invisible(capture.output(bart_predict_Y1_prob <-
                               predict(bart_ymodel, newdata =
                                         data.frame(A = A0, logE = logE0, L2 = L20, t00 = 1, t01 = 0, t02 = 0,
    ...
    invisible(capture.output(bart_predict_logE1_mean <-
                               predict(bart_emodel, newdata =
                                         data.frame(lag_A = A0, lag_logE = logE0, lag_L2 = L20, t01 = 1, t02 = 0,
                                                    t03 = 0, t04 = 0))[1,]))
    ...
    invisible(capture.output(bart_covmodel2_predict_t1_L2_mean <-
                               predict(bart_covmodel2, newdata =
                                         data.frame(lag_A = A0, lag_logE = logE0, logE = logE1, lag_L2 = L20,
    ...
    invisible(capture.output(bart_predict_Y2_prob <-
                               predict(bart_ymodel, newdata =
                                         data.frame(A = A1, logE = logE1, L2 = L21, t00 = 0, t01 = 1, t02 = 0,
                                                    t03 = 0, t04 = 0))$prob.test.mean))
    ## generate Y2
    ...
    invisible(capture.output(bart_predict_logE2_mean <-
                               predict(bart_emodel, newdata =
                                         data.frame(lag_A = A1, lag_logE = logE1, lag_L2 = L21, t01 = 0, t02 = 1,
                                                    t03 = 0, t04 = 0))[1,]))
    ...
    invisible(capture.output(bart_covmodel2_predict_t2_L2_mean <-
                               predict(bart_covmodel2, newdata =
                                         data.frame(lag_A = A1, lag_logE = logE1, logE = logE2, lag_L2 = L21,
    ...
    invisible(capture.output(bart_predict_Y3_prob <-
                               predict(bart_ymodel, newdata =
                                         data.frame(A = A2, logE = logE2, L2 = L22, t00 = 0, t01 = 0, t02 = 1,
                                                    t03 = 0, t04 = 0))$prob.test.mean))
    ## generate Y3
    ...
    invisible(capture.output(bart_predict_logE3_mean <-
                               predict(bart_emodel, newdata =
                                         data.frame(lag_A = A2, lag_logE = logE2, lag_L2 = L22, t01 = 0, t02 = 0,
                                                    t03 = 1, t04 = 0))[1,]))
    ...
    invisible(capture.output(bart_covmodel2_predict_t3_L2_mean <-
                               predict(bart_covmodel2, newdata =
                                         data.frame(lag_A = A2, lag_logE = logE2, logE = logE3, lag_L2 = L22,
    ...
    invisible(capture.output(bart_predict_Y4_prob <-
                               predict(bart_ymodel, newdata =
                                         data.frame(A = A3, logE = logE3, L2 = L23, t00 = 0, t01 = 0, t02 = 0,
    ...
    invisible(capture.output(bart_predict_logE4_mean <-
                               predict(bart_emodel, newdata =
                                         data.frame(lag_A = A3, lag_logE = logE3, lag_L2 = L23, t01 = 0, t02 = 0,
                                                    t03 = 0, t04 = 1))[1,]))
    ...
    invisible(capture.output(bart_covmodel2_predict_t4_L2_mean <-
                               predict(bart_covmodel2, newdata =
                                         data.frame(lag_A = A3, lag_logE = logE3, logE = logE4, lag_L2 = L23,
    ...
    invisible(capture.output(bart_predict_Y5_prob <-
                               predict(bart_ymodel, newdata =
                                         data.frame(A = A4, logE = logE4, L2 = L24,
